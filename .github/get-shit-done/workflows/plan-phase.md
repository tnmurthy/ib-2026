...
TDD_MODE=$(node ".github/get-shit-done/bin/gsd-tools.cjs" config-get workflow.tdd_mode 2>/dev/null || echo "false")
```
...
AI_PHASE_CFG=$(node ".github/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```
...
Check config:
```bash
PATTERN_MAPPER_CFG=$(node ".github/get-shit-done/bin/gsd-tools.cjs" config-get workflow.pattern_mapper --default true 2>/dev/null)
```
...
**Read pass count:**
```bash
BOUNCE_PASSES=$(node ".github/get-shit-done/bin/gsd-tools.cjs" config-get workflow.plan_bounce_passes --default 2)
BOUNCE_SCRIPT=$(node ".github/get-shit-done/bin/gsd-tools.cjs" config-get workflow.plan_bounce_script)
```
...
```bash
node ".github/get-shit-done/bin/gsd-tools.cjs" commit "refactor(${padded_phase}): bounce plans through external refinement" --files "${PHASE_DIR}/*-PLAN.md"
```
