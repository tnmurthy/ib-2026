...
```
UI_PHASE_FLAG=$(node ".github/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
```bash
CHECKPOINT=$(node ".github/get-shit-done/bin/gsd-tools.cjs" uat render-checkpoint --file "$uat_path" --raw)
...
```bash
node ".github/get-shit-done/bin/gsd-tools.cjs" audit-open --json 2>/dev/null
```
